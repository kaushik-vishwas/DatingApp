...
function getWithdrawalNotifySmtpAuth(): { user: string; pass: string } | null {
...
function buildTransporterFromAuth(auth: { user: string; pass: string }): Transporter {
...
export type WithdrawalRequestEmailPayload = {
  withdrawalId: string;
  receiverName: string;
  amount: number;
  payoutAmount: number;
...
export const sendWithdrawalRequestAdminEmail = async (
  ...
  if (!auth) {
    console.warn(
      '[email] withdrawal notify skipped: set WITHDRAWAL_NOTIFY_EMAIL and WITHDRAWAL_NOTIFY_APP_PASS in backend/.env'
    );
    return;
  }
  ...
  const accountLinesText: string[] = [`Method: ${methodLabel}`, `Account holder: ${holder}`];

  if (method === 'upi' || upi) {
    accountLinesHtml.push(
      `<tr><td style="padding:6px 0;color:#6b7280;font-size:13px;">UPI ID</td><td style="padding:6px 0;text-align:right;color:#111827;font-size:13px;font-weight:700;word-break:break-all;">${escapeHtml(upi || '—')}</td></tr>`
    );
    accountLinesText.push(`UPI ID: ${upi || '—'}`);
  }
  if (method === 'bank' || accountNumber || ifsc) {
    accountLinesHtml.push(
      `<tr><td style="padding:6px 0;color:#6b7280;font-size:13px;">Account number</td><td style="padding:6px 0;text-align:right;color:#111827;font-size:13px;font-weight:700;">${escapeHtml(accountNumber || '—')}</td></tr>`,
      `<tr><td style="padding:6px 0;color:#6b7280;font-size:13px;">IFSC</td><td style="padding:6px 0;text-align:right;color:#111827;font-size:13px;font-weight:600;">${escapeHtml(ifsc || '—')}</td></tr>`
    );
    ...
    if (bankName && bankName.toUpperCase() !== 'BANK' && bankName.toUpperCase() !== 'UPI') {
      accountLinesHtml.push(
        `<tr><td style="padding:6px 0;color:#6b7280;font-size:13px;">Bank</td><td style="padding:6px 0;text-align:right;color:#111827;font-size:13px;font-weight:600;">${escapeHtml(bankName)}</td></tr>`
      );
