...
exports.getAppUpdatePolicy = void 0;
/**
 ...
 */
const getAppUpdatePolicy = async (_req, res) => {
    try {
        const enabled = process.env.APP_UPDATE_ENABLED?.trim().toLowerCase() === 'true';
        const minAndroidVersionCode = Math.max(0, Number(process.env.APP_UPDATE_MIN_ANDROID_VERSION_CODE ?? 0) || 0);
        const latestAndroidVersionCode = Math.max(minAndroidVersionCode, Number(process.env.APP_UPDATE_LATEST_ANDROID_VERSION_CODE ?? minAndroidVersionCode) ||
...
exports.getAppUpdatePolicy = getAppUpdatePolicy;
