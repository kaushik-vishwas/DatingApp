...
const completeProfile = async (req, res) => {
    ...
    try {
        ...
        const aadhaarFrontUrl = typeof aadhaarFront === 'string' && aadhaarFront.trim() ? aadhaarFront.trim() : null;
        const aadhaarBackUrl = typeof aadhaarBack === 'string' && aadhaarBack.trim() ? aadhaarBack.trim() : null;
        const aadhaarDigits = typeof aadhaarNumber === 'string' ? aadhaarNumber.replace(/\D/g, '').trim() : '';
        ...
        receiver.documents = [aadhaarFrontUrl, aadhaarBackUrl, panFrontUrl].filter((u) => Boolean(u));
...
const saveReceiverKycBankFinalize = async (req, res) => {
    ...
    try {
        ...
        receiver.documents = [front, back, panFrontUrl].filter((u) => Boolean(u));
        receiver.bankAccountHolderName = String(bankAccountHolderName).trim();
        receiver.bankAccountType = bankAccountType;
        receiver.bankAccountNumber = String(bankAccountNumber).trim();
        receiver.bankIfsc = String(bankIfsc).trim().toUpperCase();
        ...
        t.json(200, { message: 'Bank details saved', user: (0, authController_1.toApiReceiver)(receiver) });
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
 ...
 */
const saveReceiverUserAudio = async (req, res) => {
    try {
        if (req.accountKind !== 'receiver') {
            res.status(403).json({
                message: 'This endpoint is only for receiver accounts',
...
const getReceiverWithdrawalOverview = async (req, res) => {
    try {
        ...
        const recentRows = await WithdrawalRequest_1.default.find({
            ...
            .sort({ createdAt: -1 })
            .limit(20)
            .select('amount platformFee payoutAmount status createdAt payoutStatus payoutUtr')
            .lean();
        ...
        res.status(200).json({
            ...
            minWithdrawalInr: receiverWithdrawalFees_1.RECEIVER_MIN_WITHDRAWAL_INR,
            platformFeePercent: receiverWithdrawalFees_1.RECEIVER_WITHDRAWAL_PLATFORM_FEE_PERCENT,
            payment: {
                nameAsPerAadhaar: receiver.nameAsPerAadhaar ?? '',
                upiMasked: receiver.upiId ? maskUpiId(receiver.upiId) : '',
                complete: (0, receiverPaymentDetails_1.receiverPaymentDetailsComplete)(receiver),
                payoutMethod: receiver.upiId ? 'upi' : receiver.bankAccountNumber ? 'bank' : undefined,
            },
            bank: {
            ...
            recent: recentRows.map((row) => ({
                ...
                platformFee: roundInr(typeof row.platformFee === 'number' ? row.platformFee : 0),
                payoutAmount: roundInr((0, receiverWithdrawalFees_1.resolveWithdrawalPayoutAmount)(row)),
                status: row.status,
...
const sendReceiverWithdrawalOtp = async (req, res) => {
    try {
        ...
        if (!(0, receiverWithdrawalFees_1.isValidReceiverWithdrawalAmount)(amount)) {
            res.status(400).json({ message: `Minimum withdrawal is ₹${receiverWithdrawalFees_1.RECEIVER_MIN_WITHDRAWAL_INR}` });
            return;
        }
        const breakdown = (0, receiverWithdrawalFees_1.computeReceiverWithdrawalBreakdown)(amount);
        if (breakdown.netPayout < 1) {
            res.status(400).json({ message: 'Withdrawal amount is too low after platform fee' });
            return;
        }
        const rid = String(req.receiver._id);
        ...
        if (breakdown.requestedAmount > withdrawable.requestableBalance) {
            res.status(400).json({
                message: withdrawable.pendingAmount > 0
                    ? 'You already have a pending withdrawal. Wait until it is paid or rejected before requesting more.'
                    : 'Insufficient wallet balance',
            });
            return;
        }
        if (!(0, receiverPaymentDetails_1.receiverPaymentDetailsComplete)(receiver)) {
        ...
        const issued = await (0, smsOtp_1.issueSmsOtp)(receiver.phone, 'withdrawal');
        await WithdrawalRequest_1.default.findOneAndUpdate({ receiverId: rid, status: 'verification_pending' }, {
            $set: {
                amount: breakdown.requestedAmount,
                platformFee: breakdown.platformFee,
                payoutAmount: breakdown.netPayout,
                payoutMethod,
                status: 'verification_pending',
                verificationCodeHash: (0, smsOtp_1.encodeStoredSmsOtp)(issued),
        ...
        if (err instanceof smsOtp_1.MessageCentralError) {
            res.status((0, messageCentral_1.httpStatusForMessageCentral)(err)).json({
                message: err.message,
...
const verifyReceiverWithdrawalOtpAndCreate = async (req, res) => {
    try {
        ...
        const [receiver, pendingVerification] = await Promise.all([
            Receiver_1.default.findById(rid).select('walletBalance name nameAsPerAadhaar bankAccountHolderName upiId bankAccountNumber bankIfsc bankName'),
        ...
        try {
            await (0, smsOtp_1.assertSmsOtpValid)({
            ...
            if (e instanceof smsOtp_1.MessageCentralError) {
                if (e.responseCode === 705) {
                    await WithdrawalRequest_1.default.deleteOne({ _id: pendingVerification._id });
                }
                res.status((0, messageCentral_1.httpStatusForMessageCentral)(e)).json({
                    message: e.message,
        ...
        void (0, email_1.sendWithdrawalRequestAdminEmail)({
            ...
            payoutMethod: pendingVerification.payoutMethod ?? null,
            accountHolderName: pendingVerification.accountHolderName ||
...
const sendReceiverBankUpdateOtp = async (req, res) => {
    try {
        ...
        if ((0, accountAccess_1.blockReceiverUntilApproved)(req, res))
            return;
        const parsed = (0, receiverPaymentDetails_1.parseReceiverPaymentUpdateBody)(req.body);
        if ('error' in parsed) {
        ...
        const issued = await (0, smsOtp_1.issueSmsOtp)(receiver.phone, 'bank_update');
        receiver.otp = (0, smsOtp_1.encodeStoredSmsOtp)(issued);
        ...
        receiver.pendingPanNumber = parsed.pan;
        receiver.pendingBankAccountNumber = parsed.bankAccountNumber;
        receiver.pendingBankIfsc = parsed.bankIfsc;
        receiver.pendingBankAccountHolderName = parsed.bankAccountHolderName;
        receiver.pendingBankAccountType = parsed.bankAccountNumber ? 'savings' : null;
...
const verifyReceiverBankUpdateOtp = async (req, res) => {
    try {
        ...
        if (!(0, receiverPaymentDetails_1.receiverPaymentDetailsComplete)({
            nameAsPerAadhaar: receiver.pendingNameAsPerAadhaar,
            upiId: receiver.pendingUpiId,
            aadhaarNumber: receiver.pendingAadhaarNumber,
            panNumber: receiver.pendingPanNumber,
            bankAccountNumber: receiver.pendingBankAccountNumber,
            bankIfsc: receiver.pendingBankIfsc,
            bankAccountHolderName: receiver.pendingBankAccountHolderName,
        })) {
            res.status(400).json({ message: 'No pending payment details found. Start again' });
            return;
        }
        ...
        receiver.bankAccountType = receiver.pendingBankAccountNumber ? 'savings' : null;
        if (!receiver.pendingUpiId)
            receiver.upiId = null;
        if (!receiver.pendingBankAccountNumber) {
            receiver.bankAccountNumber = null;
            receiver.bankIfsc = null;
            receiver.bankAccountHolderName = null;
