  ...
  function normalizeChipSelection(list: string[], options: string[], max: number): string[] {
  ...
  export default function ReceiverEditProfileScreen(): React.JSX.Element {
    ...
    const [languages, setLanguages] = useState<string[]>(() =>
      normalizeChipSelection(user?.languages ?? [], LANGUAGE_OPTIONS, MAX_LANGUAGES),
    );
    const [interests, setInterests] = useState<string[]>(() =>
      normalizeChipSelection(user?.interests ?? [], INTEREST_OPTIONS, MAX_INTERESTS),
    );

    useEffect(() => {
      if (!user) return;
      setLanguages(normalizeChipSelection(user.languages ?? [], LANGUAGE_OPTIONS, MAX_LANGUAGES));
      setInterests(normalizeChipSelection(user.interests ?? [], INTEREST_OPTIONS, MAX_INTERESTS));
    }, [user]);
    ...
    const onSave = async () => {
      ...
      if (isWithdrawKycMode) {
        ...
        if (interests.length > MAX_INTERESTS) {
          Alert.alert('Validation', `You can select up to ${MAX_INTERESTS} interests.`);
          return;
        }
        if (languages.length > MAX_LANGUAGES) {
          Alert.alert('Validation', `You can select up to ${MAX_LANGUAGES} languages.`);
          return;
        }
        if (!profileImageUri) {
          ...
          >
            ...
            {isWithdrawKycMode ? (
              ...
              <View style={styles.formSection}>
                ...
                {/* Languages */}
                <Text style={styles.fieldLabel}>Languages <Text style={styles.requiredStar}>*</Text> (max 2)</Text>
                <ChipGroup
      ...
      {options.map((opt) => {
        ...
        return (
          <TouchableOpacity
            ...
            onPress={() => {
              if (active) {
                onChange(visibleSelected.filter((x) => x !== opt));
                return;
              }
              if (visibleSelected.length >= max) {
                Alert.alert('Maximum reached', `You can only select up to ${max} ${selectionLabel}`);
                return;
              }
              onChange([...visibleSelected, opt]);
            }}
            activeOpacity={0.7}
          >
