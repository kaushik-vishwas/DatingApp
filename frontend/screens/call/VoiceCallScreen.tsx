...
export default function VoiceCallScreen({ navigation, route }: Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      void (async () => {
        try {
          await callApi.sessionEnd(route.params.callId);
        } catch {
          // ignore session end failures on unmount
        }
        try {
  ...
  const hangup = async () => {
    ...
    if (user?.role === 'caller') {
      Alert.alert('Rate this call', 'How was the call quality?', [
        ...
        { text: '3', onPress: async () => { try { await callApi.sessionRate(route.params.callId, 3); } catch { /* ignore */ } navigation.goBack(); } },
        { text: '4', onPress: async () => { try { await callApi.sessionRate(route.params.callId, 4); } catch { /* ignore */ } navigation.goBack(); } },
        { text: '5', onPress: async () => { try { await callApi.sessionRate(route.params.callId, 5); } catch { /* ignore */ } navigation.goBack(); } },
      ]);
      return;
    }
    navigation.goBack();
