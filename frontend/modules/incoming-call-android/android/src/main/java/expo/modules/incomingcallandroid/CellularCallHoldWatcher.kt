...
/**
 * Detects cellular call interruption via [AudioManager] mode and telephony off-hook
 * (when READ_PHONE_STATE is granted), plus (API 31+) mode-change callbacks.
 *
 * Listen-only: does NOT request AudioFocus. Competing with the phone app for
 * STREAM_VOICE_CALL focus while answering cellular is a common process-kill vector.
 *
...
object CellularCallHoldWatcher {
  ...
  private const val POLL_MS_HOLD = 400L
  ...
  fun start(context: Context, onActiveChanged: (Boolean, Int, String) -> Unit) {
    ...
    // Do NOT request AudioFocus — competing with the phone app for STREAM_VOICE_CALL
