...
/** Page 2: Aadhaar optional; PAN required. If Aadhaar number is entered it must be 12 digits. */
export function validateAadhaarDocuments(state: CompleteProfileState): string | null {
  const aadhaar = state.aadhaarNumber.trim();
  if (aadhaar && !/^\d{12}$/.test(aadhaar)) return 'Enter a valid 12-digit Aadhaar number';
